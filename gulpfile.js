...
gulp.task('mocks', function () {
  return (gulp.src('./mocks/**/*').pipe(gulp.dest(config.build + 'mocks')));
});
...
gulp.task('images', ['clean-images'], function() {
  ...
  return gulp.src(config.images).pipe(gulp.dest(config.build + 'images'));
});
 ...
 */
gulp.task('build', ['optimize', 'images', 'fonts', 'mocks'], function() {
...
function startBrowserSync(isDev, specRunner) {
  ...
  if (isDev) {
    gulp.watch([config.sass], ['styles']).on('change', changeEvent);
  } else {
    gulp.watch([config.sass, config.js, config.html], ['browserSyncReload']).on('change', changeEvent);
  }

  var options = {
    proxy: 'localhost:' + port,
    port: 3000,
    files: isDev ? [config.client + '**/*.*', '!' + config.sass, config.temp + '**/*.css'] : [],
    watchOptions: {
